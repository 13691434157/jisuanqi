...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View btn_sqrt = findViewById(R.id.btn_sqrt);//回格
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()){
            ...
            case R.id.btn_div:
                ...
                if(str.substring(str.length()-1).equals("+") || str.substring(str.length()-1).equals("*")        //不能连着两个符号
    ...
    public boolean handleSqrtLegal(String str) {   //是否合法
        int i = 0;
        if (str.charAt(1) == '√') {
        } else {
            return false;
    ...
    public double handleSpace(String str) {             //根号处理空格，格式
        int i = 2;
        if (str.indexOf("√") != -1)
            i = 2;
        ...
        for(int i =0 ; i < postList.size(); i++) {
            if (isNum(postList.get(i))) {
                ...
                if ("+".equals(postList.get(i))) {
                    ...
                    BigDecimal b1 = new BigDecimal(numStack.pop());
                    BigDecimal b2 = new BigDecimal(numStack.pop());
                    BigDecimal b3 = b1.add(b2);//此处举例使用乘法（注意：必须使用一个新的BigDecimal来获得运算后的值）
                    numStack.push(b3.toString());
                    ...
                    if (numStack.isEmpty()) {
                        ...
                        b2 = new BigDecimal(numStack.pop());
                    }
                    ...
                    Double b1 = Double.parseDouble(numStack.pop());
                    Double b2 = Double.parseDouble(numStack.pop());
                    double b3 = MyUtils.divide(b2,b1,2);//此处举例使用乘法（注意：必须使用一个新的BigDecimal来获得运算后的值）
                    numStack.push(b3 + "");
                }
            }
        }
